...
class CustomDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, tgt_mask=None, weights=None):
        # Self-attention layer
        tgt2 = F.scaled_dot_product_attention(tgt, tgt, tgt, tgt_mask)
        tgt = tgt + self.dropout1(tgt2)
        tgt = self.norm1(tgt)

        # Cross attention with the 
        if weights is not None:
            weights_repeated = weights.unsqueeze(-1).repeat(1, 1, 16)
            tgt3 = F.scaled_dot_product_attention(tgt, weights_repeated, weights_repeated) 
...
class CustomTransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, tgt_mask=None, weights=None):
        ...
        for layer in self.layers:
            output = layer(output, tgt_mask, weights)
...
class Actor(nn.Module):
    ...
    def __init__(self, device, params, des_space, comp_list):
    ...
    def forward(self, x, ind, weights):
        ...
        with autocast(device_type=self.device.type, dtype=torch.float16):
            ...
            x = self.transformer_decoder(x, tgt_mask=mask, weights=weights)
    ...
    def sample_action(self, observation, ind, weights):
        ...
        if ind >= len(self.des_space):
            ...
            out_head = ind
        output = self.forward(input_observation, out_head, weights)
    ...
    def ppo_update(self, observation, action, logprob, advantage, weights):
        ...
        with autocast(device_type=self.device.type, dtype=torch.float16):
            ...
            for i in range(len(self.des_space)):
                ...
                if i < 5:
                    ...
                    rep_weights = weights.repeat_interleave(len(self.comp_list), dim=0)
                ...
                if self.des_space[i]['type'] == 'continuous':
                    params = self.forward(var_observation, i, rep_weights)
                    ...
                    pred_probs = self.forward(var_observation, i, rep_weights)[:,-1,:]
