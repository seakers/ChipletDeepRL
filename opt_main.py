...
def main():
    ...
    for run in range(num_runs):
        ...
        print(f"\n\nStarting run {run + 1}/{num_runs} for trace {trace}\n\n")

        # Run Random Search
        all_des_rs, all_obj_rs, pareto_front_des_rs, pareto_front_obj_rs, hypervolumes_rs, NFE_rs, max_values = run_random_search(params)
        ...
        all_des_ga, all_obj_ga, pareto_front_des_ga, pareto_front_obj_ga, hypervolumes_ga, NFE_ga = run_genetic_algorithm(max_values, params)
        ...
        all_des_rl, all_obj_rl, pareto_front_des_rl, pareto_front_obj_rl, hypervolumes_rl, NFE_rl = run_ppo_optimization(max_values, params)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"results/{params['date_str']}/hypervolume_plot_{trace}.png")

    # Plot the first, middle, and last Pareto front for Random Search
    num_pareto_fronts_rs = len(all_runs_pareto_front_obj_rs[0])
    middle_index_rs = num_pareto_fronts_rs // 2

    plt.figure()
    for run_idx, pareto_front_obj_rs in enumerate(all_runs_pareto_front_obj_rs):
        if num_pareto_fronts_rs > 0:
            plt.scatter(*zip(*pareto_front_obj_rs[0]), label=f'RS Run {run_idx + 1} - First Pareto Front', color='blue', alpha=0.7, marker='X')
        if num_pareto_fronts_rs > 1:
            plt.scatter(*zip(*pareto_front_obj_rs[middle_index_rs]), label=f'RS Run {run_idx + 1} - Middle Pareto Front', color='blue', alpha=0.7, marker='+')
        if num_pareto_fronts_rs > 2:
            plt.scatter(*zip(*pareto_front_obj_rs[-1]), label=f'RS Run {run_idx + 1} - Last Pareto Front', color='blue', alpha=0.7)

    # Plot the first, middle, and last Pareto front for Genetic Algorithm
    num_pareto_fronts_ga = len(all_runs_pareto_front_obj_ga[0])
    middle_index_ga = num_pareto_fronts_ga // 2

    for run_idx, pareto_front_obj_ga in enumerate(all_runs_pareto_front_obj_ga):
        if num_pareto_fronts_ga > 0:
            plt.scatter(*zip(*pareto_front_obj_ga[0]), label=f'GA Run {run_idx + 1} - First Pareto Front', color='orange', alpha=0.7, marker='x')
        if num_pareto_fronts_ga > 1:
            plt.scatter(*zip(*pareto_front_obj_ga[middle_index_ga]), label=f'GA Run {run_idx + 1} - Middle Pareto Front', color='orange', alpha=0.7, marker='+')
        if num_pareto_fronts_ga > 2:
            plt.scatter(*zip(*pareto_front_obj_ga[-1]), label=f'GA Run {run_idx + 1} - Last Pareto Front', color='orange', alpha=0.7)

    # Plot the first, middle, and last Pareto front for RL
    num_pareto_fronts_rl = len(all_runs_pareto_front_obj_rl[0])
    middle_index_rl = num_pareto_fronts_rl // 2

    for run_idx, pareto_front_obj_rl in enumerate(all_runs_pareto_front_obj_rl):
        if num_pareto_fronts_rl > 0:
            plt.scatter(*zip(*pareto_front_obj_rl[0]), label=f'RL Run {run_idx + 1} - First Pareto Front', color='green', alpha=0.7, marker='o')
        if num_pareto_fronts_rl > 1:
            plt.scatter(*zip(*pareto_front_obj_rl[middle_index_rl]), label=f'RL Run {run_idx + 1} - Middle Pareto Front', color='green', alpha=0.7, marker='+')
        if num_pareto_fronts_rl > 2:
            plt.scatter(*zip(*pareto_front_obj_rl[-1]), label=f'RL Run {run_idx + 1} - Last Pareto Front', color='green', alpha=0.7)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f"results/{params['date_str']}/pareto_fronts_{trace}.png")
    ...
    plt.tight_layout()
    plt.savefig(f"results/{params['date_str']}/boxplot_designs_{trace}.png")
