...
class Actor(nn.Module):
    ...
    def __init__(self, device, params, des_space, comp_list, num_objectives):
    ...
    def get_valid_panel_mask(self, observation_tensor, des_space, ind):
        ...
        """
        Updated to handle observations that include Pareto front state at the beginning.
        
        observation_tensor: (batch_size, seq_len) tensor where seq_len starts with Pareto front state
                            followed by actions. We need to extract structure_id and shelves from the action part.
        """
        batch_size = observation_tensor.size(0)
        num_panel_options = len(des_space[5]['range'])
        max_pareto_size = 100  # Same as defined in PPO
        pareto_state_size = max_pareto_size * self.num_objectives
        
        # Extract structure_id and shelves from the action part of the observation
        # structure_id is at index pareto_state_size + 0, shelves at pareto_state_size + 4
        structure_ids = observation_tensor[:, pareto_state_size + 0].long()
        shelves = observation_tensor[:, pareto_state_size + 4].long()
    ...
    def sample_action(self, observation, ind):
        ...
        if ind >= len(self.des_space):
            out_head = ((ind-len(self.des_space)) % 5) + 5
        else:
            out_head = ind
        output = self.forward(input_observation, out_head)
        ...
        if self.des_space[out_head]['type'] == 'continuous':
            ...
            if ind == 5 or (ind > 5 and (ind - 5) % 5 == 0):  # panel choice indices
                valid_mask = self.get_valid_panel_mask(input_observation, self.des_space, ind).to(self.device)
    ...
    def ppo_update(self, observation, action, logprob, advantage):
        ...
        with autocast(device_type=self.device.type, dtype=torch.float16):
            max_pareto_size = 100  # Same as defined in PPO
            pareto_state_size = max_pareto_size * self.num_objectives
            num_vars = observation.size(1) - pareto_state_size  # Subtract Pareto front state size
...
class Critic(nn.Module):
    ...
    def __init__(self, device, params, num_objectives, input_dim):
        ...
        self.output_layer = nn.Linear(self.dense_dim, 1)  # Keep as 1 for HV reward
    ...
    def ppo_update(self, observation, return_tensor):

        self.optimizer.zero_grad()
        with autocast(device_type=self.device.type, dtype=torch.float16):
            new_pred_reward = self(observation)
